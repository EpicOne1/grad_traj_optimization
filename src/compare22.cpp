...
#include "grad_traj_optimization/kinodynamic_astar.h"
...
int main(int argc, char** argv)
{
  ...
  // -------------------ros initialization---------------------
  setpoint_pub = node.advertise<visualization_msgs::Marker>("grad_opt/setpoint", 10);
  traj_point_pub = node.advertise<visualization_msgs::Marker>("grad_opt/traj_point", 10);
  ...
  /* ---------- init sdf map and edt environment ---------- */
  SDFMap::Ptr sdf_map;
  ...
  /* ---------- init path finder ---------- */
  KinodynamicAstar::Ptr path_finder;
  path_finder.reset(new KinodynamicAstar);
  path_finder->setParam(nh);  // need to modify launch
  path_finder->setEnvironment(edt_env);
  path_finder->init();
  ...
  while (ros::ok())
  {
    ...
    /* manage map, wait for update finish */
    if (exp_num % use_map_num == 0)
    {
      sdf_map->resetBuffer(Eigen::Vector3d(-20, -20, -2), Eigen::Vector3d(20, 20, 5));
    ...
    int status = path_finder->search(start_pt, start_vel, start_acc, end_pt, end_vel, false);
    ...
    else
    {
      ...
      for (int i = 0; i < Time.rows(); ++i)
      {
        ...
        cx[3] = Acc(i, 0) / 2.0, cy[3] = Acc(i, 1) / 2.0, cz[3] = Acc(i, 2) / 2.0;
      ...
      displayPathWithColor(traj_vis_path, 0.1, Eigen::Vector4d(1, 0, 0, 1), 2);

      double acc_cost = poly_traj1.getAccCost();
      double time_sum_path = poly_traj1.getTimeSum();
      cout << "test2:" << exp_num + 1 << "solve_time:" << time_search << ",traj_time:" << time_sum_path
           << ",acc_cost:" << acc_cost << endl;

      std::ofstream file("/home/bzhouai/workspaces/plan_ws/src/uav_planning_bm/resource/"
                         "front2.txt",
                         std::ios::app);
      if (file.fail())
      {
      ...
      file << "test2:" << exp_num + 1 << ",solve_time:" << time_search << ",traj_time:" << time_sum_path
           << ",acc_cost:" << acc_cost << "\n";
      file.close();
      // ros::Duration(0.5).sleep();
    }
