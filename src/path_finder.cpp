...
/*      std::vector<int> indices;
      ...
      kdtreeForMap.setInputCloud( CloudIn.makeShared() );

      //ROS_WARN("Input Set OK .. ");
     /* ROS_WARN("Check points number of pointCloud : %lu", CloudIn.points.size());
      ros::Time time2 = ros::Time::now();
      cout<<"time input is: "<<(time2-time1).toSec()<<endl;
*/}
...
inline int  rrtPathFinder::CheckRelation( float dis, NodePtr node_1, NodePtr node_2 )
{
      ...
      else if( 1.1 * dis < (node_1->radius + node_2->radius) ) // && (dis > node_1->radius) && (dis > node_2->radius)
          status = -1;
      else
          status = 0;
...
void rrtPathFinder::localReConnect(NodePtr curPtr)
{
      // Local rewire of a given node, reconnect it with nearby nodes
      //ROS_WARN("[Path Finder] path re-evaluate, recheck connection, start the reConnect");
      float range = curPtr->radius * 2.0f;
      float pos[3] = {curPtr->coord.x, curPtr->coord.y, curPtr->coord.z};
      struct kdres *presults  = kd_nearest_rangef( kdTree_, pos, range);

      vector<NodePtr> nearPtrList;
      if(int(kd_res_size(presults)) == 0){
          //ROS_ERROR("[Path Re-evaluate] no nearby nodes around it");
          return;
      }
      else{
          while( !kd_res_end( presults ) ) { // Pull all the nodes outside the result data structure from the kd-tree
      ...
      //ROS_WARN("[Path Finder] path re-evaluate, recheck connection, finish search nearby nodes");

      float min_cost = curPtr->g;
      vector<NodePtr> nearVertex;
      // Go through all the nearby vertex again, to deal with another two case. Note, better not mix this go through with the previous one
      NodePtr nearPtr = NULL;
      NodePtr lstParentPtr = NULL;
      
      for(auto nodeptr:nearPtrList){
          nearPtr = nodeptr;
          if(! nearPtr->valid ) continue;
          
          // Choose the parent
          float dis = getDis( nearPtr, curPtr );
          int   res = CheckRelation( dis, nearPtr, curPtr );
          if( res == -1 ){
              float cost = nearPtr->g + dis;
              if( cost < min_cost ){ // has a shorter path if new_ptr's father is the new node
                  min_cost = cost;
                  
                  if(isSuccessor(curPtr, nearPtr)) 
                    continue;

                  if(curPtr->preNode_ptr==NULL){ // curPtr has no father, now it has one, record the father node 
                      //ROS_ERROR("choose 1st father");
                      lstParentPtr = nearPtr;  
                      lstParentPtr->nxtNode_ptr.push_back(curPtr);
                  }
                  else{ // curPtr already has a father, now it find a better one, change it and record it
                      /*ROS_ERROR("change a father");
                      cout<<"child size: "<<int(lstParentPtr->nxtNode_ptr.size())<<endl;  */
                      lstParentPtr->nxtNode_ptr.pop_back();
                      lstParentPtr = nearPtr; 
                      lstParentPtr->nxtNode_ptr.push_back(curPtr);
                  }
                  curPtr->preNode_ptr = nearPtr;
                  curPtr->g = min_cost;
                  //ROS_ERROR("finish set the father");
              }
              nearVertex.push_back(nearPtr);
          }
      }

      //ROS_WARN("[Path Finder] path re-evaluate, recheck connection, finish choose the parent node");
      // *** Rewire the neighbor *** //
      for(int i = 0; i < int(nearVertex.size()); i++ ){
          NodePtr nodeptr = nearVertex[i];
          
          if(!nodeptr->valid)  continue;
          
          NodePtr nearPtr = nodeptr;
          float dis = getDis( nearPtr, curPtr );
          float cost = dis + curPtr->g;
          
          if( cost < nearPtr->g){ // a near Node changed a father, delete this node from its parent's childer list
              if(isSuccessor(nearPtr, curPtr)) 
                    continue;

              NodePtr lstNearParent = nearPtr->preNode_ptr;
              if(lstNearParent == NULL) continue;
              nearPtr->preNode_ptr = curPtr;
              nearPtr->g = cost;
              ...
              nearPtr->change = false; // release the flag after the delete
              curPtr->nxtNode_ptr.push_back(nearPtr);
          }
      }

      //ROS_WARN("[Path Finder] path re-evaluate, recheck connection, finish local rewire");
}

void rrtPathFinder::ReConnect(bool isPrint)
{     
      /*ROS_WARN("[Path Finder] before local rewire all shrinked nodes");      
      ROS_WARN("[ReConnect] Size of the re-connect list is %d", int(reCheckList.size()) );*/
      vector<NodePtr> tmpReCheckList;

      for(int i = 0; i < int(reCheckList.size()); i ++ ){
          NodePtr nodeptr = reCheckList[i];
          
          //cout<<"coordinate: "<<nodeptr->coord.x<<" , "<<nodeptr->coord.y<<" , "<<nodeptr->coord.z<<endl;

          if( nodeptr == NULL ) ROS_BREAK();

          if( nodeptr->preNode_ptr != NULL )
          {
              NodePtr ptrParent = nodeptr->preNode_ptr; 
              parentForgetChild( ptrParent, nodeptr );
              nodeptr->preNode_ptr = NULL;
              nodeptr->g = inf;
          }
          nodeptr->radius = radius_search(nodeptr->coord);
          
          if(nodeptr->radius < safety_margin ) // The node is now in-valid and should be deleted
          {
              if(nodeptr->valid){
                 nodeptr->valid = false;
                 invalidSet.push_back(nodeptr); 
             ...
             clearBranchS(nodeptr);
              ...
              localReConnect(nodeptr);

              if(nodeptr->preNode_ptr == NULL ) // If we can't find a father node for it, still record it for next turn
                tmpReCheckList.push_back(nodeptr); // The order ? last in first out? consider we shoudl first re-connect the new inspected nodes
          }
      }

      reCheckList = tmpReCheckList;

      /*ROS_WARN("[Path Finder] finish local rewire all shrinked nodes");
      ROS_WARN("[ReConnect] Size of the re-connect list is %d", int(reCheckList.size()) );
      for(int i = 0; i < int(reCheckList.size()); i ++ ){
          NodePtr nodeptr = reCheckList[i];
          cout<<"coordinate: "<<nodeptr->coord.x<<" , "<<nodeptr->coord.y<<" , "<<nodeptr->coord.z<<endl;
