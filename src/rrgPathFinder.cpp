...
rrgPathFinder::rrgPathFinder(double xl, double xh, double yl, double yh,
                             double zl, double zh, double biasl, double biash) {
...
void rrgPathFinder::setInput(pcl::PointCloud<pcl::PointXYZ> CloudIn) {
  ...
  ROS_WARN("Check points number of pointCloud : %lu",
           Cloud_DS_NaN.points.size());
...
double rrgPathFinder::getDis(const NodePtr node1, const NodePtr node2) {
  return sqrt(pow(node1->coord.x - node2->coord.x, 2) +
...
double rrgPathFinder::getDis(const NodePtr node1, const Point pt) {
  return sqrt(pow(node1->coord.x - pt.x, 2) + pow(node1->coord.y - pt.y, 2) +
              pow(node1->coord.z - pt.z, 2));
}

double rrgPathFinder::getDisL1(const NodePtr node1, const Point pt) {
  return fabs(node1->coord.x - pt.x) + fabs(node1->coord.y - pt.y) +
         fabs(node1->coord.z - pt.z);
}

double rrgPathFinder::radius_search(
    Point search_Pt)  //, pcl::search::KdTree<pcl::PointXYZ>::Ptr Tree)
{
  ...
  kdtreeForMap.nearestKSearch(searchPoint, 1, pointIdxRadiusSearch,
                              pointRadiusSquaredDistance);
  double radius = sqrt(pointRadiusSquaredDistance[0]);  // The height of flight
                                                        // yard is about 2.6 m
...
Point rrgPathFinder::genSample() {
  ...
  if (bias > 0.50 && bias < 0.525) pt = end_pt;
...
NodePtr rrgPathFinder::findNearst(Point pt_sample) {
  ...
  for (int i = -1; i < 2; i++)
    for (int j = -1; j < 2; j++) {
      ...
      for (vector<NodePtr>::iterator iter = NodeMap[_row][_col].begin();
           iter != NodeMap[_row][_col].end(); ++iter) {
...
NodePtr rrgPathFinder::findNearst(Point pt_sample, vector<NodePtr> NodeList) {
  ...
  for (vector<NodePtr>::iterator iter = NodeList.begin();
...
NodePtr rrgPathFinder::genNewNode(Point pt_sample, NodePtr node_nearst_ptr) {
  ...
  node_new_center.x =
      node_nearst_ptr->coord.x +
      (pt_sample.x - node_nearst_ptr->coord.x) * node_nearst_ptr->radius / dis;
  node_new_center.y =
      node_nearst_ptr->coord.y +
      (pt_sample.y - node_nearst_ptr->coord.y) * node_nearst_ptr->radius / dis;
  node_new_center.z =
      node_nearst_ptr->coord.z +
      (pt_sample.z - node_nearst_ptr->coord.z) * node_nearst_ptr->radius / dis;
...
bool rrgPathFinder::check_end(NodePtr ptr) {
  ...
  if (distance + 0.1 < ptr->radius) return true;
...
void rrgPathFinder::Clear_NodeMap() {
  for (int i = 0; i < 14; i++)
    for (int j = 0; j < 14; j++) NodeMap[i][j].clear();
}
...
bool rrgPathFinder::check_no_Empty(Point pt_sample) {
  ...
  for (int i = -1; i < 2; i++)
    for (int j = -1; j < 2; j++) {
      ...
      if (!NodeMap[_row][_col].empty()) return true;
    }
...
bool rrgPathFinder::CheckConnect(double dis, NodePtr node_1, NodePtr node_2) {
  for (vector<NodePtr>::iterator iter = node_2->father_ptr.begin();
       iter != node_2->father_ptr.end(); ++iter) {
    if ((*iter)->radius == node_1->radius &&
        (*iter)->coord.x == node_1->coord.x &&
        (*iter)->coord.y == node_1->coord.y &&
        (*iter)->coord.z == node_1->coord.z)
      return false;
  }

  /*      if( dis < sqrt(abs(node_1->radius * node_1->radius - node_2->radius *
     node_2->radius)) + 1.0 ) return false;

        if((dis + 1.0) < (node_1->radius + node_2->radius))
            return true;
  */
  /*      if( dis < (1.2 * sqrt(abs(node_1->radius * node_1->radius -
     node_2->radius * node_2->radius)) + 0.25) )//0.25 ? return false;*/

  if ((dis + 0.3 < (node_1->radius + node_2->radius)) &&
      (dis > node_1->radius) && (dis > node_2->radius))
    return true;
...
void rrgPathFinder::RRGpathFind() {
  ...
  for (iter_count = 0; iter_count < 10000; iter_count++) {
    ...
    accu_time_find_nearst +=
        (time_aft_findNearest - time_bef_findNearest).toSec();
    ...
    node_new_ptr->radius =
        radius_search(node_new_ptr->coord);  //, kdtreeForMap );

    ros::Time time_aft_radiusSearch = ros::Time::now();

    accu_time_radius_search +=
        (time_aft_radiusSearch - time_bef_radiusSearch).toSec();

    if (fabs(node_new_ptr->coord.x) + fabs(node_new_ptr->radius) > 40.0 ||
        fabs(node_new_ptr->coord.y) + fabs(node_new_ptr->radius) > 40.0)
      continue;

    // if(node_new_ptr->radius < 1.5)
    if (node_new_ptr->radius < 0.5) continue;
  ...
  */
  ...
  double min_dist = inf;
  for (vector<NodePtr>::iterator iter = EndList.begin();
       iter != EndList.end() - 1; ++iter) {
    if (((*iter)->father_ptr).empty()) continue;
  ...
  for (vector<NodePtr>::iterator iter = EndList.begin(); iter != EndList.end();
       ++iter)
    if (ptr_for_Astar != (*iter)) ptr_for_Astar->father_ptr.push_back(*iter);
  ...
  if ((pow(last_x - end_pt.x, 2) + pow(last_y - end_pt.y, 2) +
       pow(last_z - end_pt.z, 2)) > last_rad * last_rad) {
...
Eigen::MatrixXd rrgPathFinder::getPath() { return Path; }

Eigen::VectorXd rrgPathFinder::getRadius() { return Radius; }