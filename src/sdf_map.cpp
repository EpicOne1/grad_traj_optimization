...
SDFMap::SDFMap(Eigen::Vector3d ori, double resolution, Eigen::Vector3d size)
{
  ...
  this->map_size_ = size;
  for (int i = 0; i < 3; ++i) grid_size_(i) = ceil(map_size_(i) / resolution_sdf_);
...
void SDFMap::resetBuffer(Eigen::Vector3d min_pos, Eigen::Vector3d max_pos)
{
  ...
  posToIndex(max_pos - Eigen::Vector3d(resolution_sdf_ / 2, resolution_sdf_ / 2, resolution_sdf_ / 2),
             max_id);
...
{
  for (int i = 0; i < 3; ++i) id(i) = floor((pos(i) - origin_(i)) * resolution_inv_);
}

void SDFMap::indexToPos(Eigen::Vector3i id, Eigen::Vector3d& pos)
{
  for (int i = 0; i < 3; ++i) pos(i) = (id(i) + 0.5) * resolution_sdf_ + origin_(i);
}
...
void SDFMap::getOccupancyMarker(visualization_msgs::Marker& m, int id, Eigen::Vector4d color)
{
  ...
  for (int x = 0; x < grid_size_(0); ++x)
    for (int y = 0; y < grid_size_(1); ++y)
      ...
      {
        if (1 != occupancy_buffer_[x * grid_size_(1) * grid_size_(2) + y * grid_size_(2) + z]) continue;
      ...
      fillESDF([&](int x) { return tmp_buffer2_[x * grid_size_(1) * grid_size_(2) + y * grid_size_(2) + z]; },
               [&](int x, double val) {
                 distance_buffer_[x * grid_size_(1) * grid_size_(2) + y * grid_size_(2) + z] =
                     min(resolution_sdf_ * std::sqrt(val),
                         distance_buffer_[x * grid_size_(1) * grid_size_(2) + y * grid_size_(2) + z]);
               },
...
double SDFMap::getMaxDistance()
{
  ...
  {
    if (distance_buffer_[i] > max_dist) max_dist = distance_buffer_[i];
  }
      ...
      for (int x = -ifn; x <= ifn; ++x)
        for (int y = -ifn; y <= ifn; ++y)
          for (int z = -ifn; z <= ifn; ++z)
          {
            ...
            if (pt_inf.z < 2.0) cloud_inflate_vis_.push_back(pt_inf);
          }
    }
  }
...
void SDFMap::init(ros::NodeHandle& nh)
{
  ...
  // cloud_sub_ =
  //     node_.subscribe<sensor_msgs::PointCloud2>("/laser_cloud_surround", 1, &SDFMap::cloudCallback, this);
...
void SDFMap::getInterpolationData(const Eigen::Vector3d& pos, vector<Eigen::Vector3d>& pos_vec,
                                  Eigen::Vector3d& diff)
{
  if (!isInMap(pos))
  {
