...
Eigen::MatrixXd TrajectoryGenerator::PolyKinoGeneration(
    const Eigen::MatrixXd &Pos, const Eigen::MatrixXd &Vel,
    const Eigen::MatrixXd &Acc, const Eigen::VectorXd &Time, const int &type) {
  m = Time.size();
  ...
  ROS_WARN("[Generator] Kino traj initialization done");
  return PolyCoeff;
}

Eigen::MatrixXd TrajectoryGenerator::PolyQPGeneration(
    const Eigen::MatrixXd &Path, const Eigen::Vector3d &Vel,
    const Eigen::Vector3d &Acc, const Eigen::VectorXd &Time,
    const int &type)  // 1 for using minumum snap for intial; 2 for using
                      // straight line for initial
{
  /*   Get initial trajectory which is a straight line( 0 zero end velocity and
   * acceleration ) minimum snap trajectory or truly minumum snap trajectory  */
...
void TrajectoryGenerator::StackOptiDep() {
  double num_f =
      6;  // 4 + 4 : only start and target position has fixed derivatives
